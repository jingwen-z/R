...
set.seed(2345)
NO <-1500
mu <- rnorm(NO,param["barmu"],sqrt(param["sigma2mu"]))
UB <- rnorm(NO,0,sqrt(param["sigma2U"]))
yB <- mu + UB 
YB <- exp(yB)
Ds <- rep(0,NO)
Ds[YB<=param["barY"]] <- 1 
epsilon <- rnorm(NO,0,sqrt(param["sigma2epsilon"]))
eta<- rnorm(NO,0,sqrt(param["sigma2eta"]))
U0 <- param["rho"]*UB + epsilon
y0 <- mu +  U0 + param["delta"]
alpha <- param["baralpha"]+  param["theta"]*mu + eta
y1 <- y0+alpha
...
plot(yB[Ds==0],y0[Ds==0],pch=1,xlim=c(4,10),ylim=c(4,10),xlab="yB",ylab="Outcomes")
points(yB[Ds==1],y0[Ds==1],pch=1,col=col.unobs)
abline(v=log(param["barY"]),col=col.unobs)
...
\begin{figure}
\centering
\includegraphics[scale=0.45]{olsestim}
\caption{Step 4}
\end{figure}
