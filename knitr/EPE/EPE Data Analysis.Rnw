...
<<Pot.obs.out.FPCI,eval=TRUE,fig.cap='Potential outcomes and observed outcomes',fig.subcap=c('Potential outcomes','Observed outcomes'),fig.align='center',out.width='.45\\textwidth',echo=FALSE,results='hide',fig.pos='htbp'>>=
col.obs <- 'black'
col.unobs <- 'red'
...
<<sample.CLT,dependson='monte.carlo',eval=FALSE,echo=FALSE,results='hide',echo=FALSE,warning=FALSE,error=FALSE,message=FALSE,fig.cap='Sample size using CLT',fig.align='center',out.width='.65\\textwidth',fig.pos='htbp'>>=
...
<<identification.sharp.ols.1,eval=FALSE,echo=FALSE,results='hide',fig.cap='Identification in a sharp RDD design',fig.align='center',out.width='.65\\textwidth',fig.pos='htbp'>>=
...
<<identification.sharp.ols.2,eval=FALSE,echo=FALSE,results='hide',fig.cap='Identification in a sharp RDD design',fig.align='center',out.width='.65\\textwidth',fig.pos='htbp'>>=
plot(yBo[Dso==0],yo[Dso==0],pch=1,xlim=c(4,10.5),ylim=c(4,10.5),xlab="yB",ylab="Outcomes")
points(yBo[Dso==1],yo[Dso==1],pch=3)
...
\begin{figure}[htb]
\centering
\includegraphics[height=8.6cm]{SRDDols}
\caption{Sharp RDD Using OLS}
\end{figure} 
...
MSE.llr.0 <- sapply(MSE.grid,MSE.llr,y=yc,D=Z,x=yBo,kernel=kernel,d=0)
xMSE.llr.1 <- sapply(MSE.grid,MSE.llr,y=yc,D=Z,x=yBo,kernel=kernel,d=1)

y.Z0.llr <- llr(yc[Z==0],yBo[Z==0],yBo[Z==0],bw=MSE.grid[MSE.llr.0==min(MSE.llr.0)],kernel=kernel)    
y.Z1.llr <- llr(yc[Z==1],yBo[Z==1],yBo[Z==1],bw=MSE.grid[MSE.llr.1==min(MSE.llr.1)],kernel=kernel)    
@
