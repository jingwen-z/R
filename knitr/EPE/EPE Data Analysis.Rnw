...
MSE.pr.llr.0 <- sapply(MSE.grid,MSE.llr,y=Ds,D=Z,x=yB,kernel=kernel,d=0)
MSE.pr.llr.1 <- sapply(MSE.grid,MSE.llr,y=Ds,D=Z,x=yB,kernel=kernel,d=1)

Pr.D0.llr <- llr(Ds[Z==0],yB[Z==0],yB[Z==0],bw=MSE.grid[MSE.pr.llr.0==min(MSE.pr.llr.0)],kernel=kernel)    
Pr.D1.llr <- llr(Ds[Z==1],yB[Z==1],yB[Z==1],bw=MSE.grid[MSE.pr.llr.1==min(MSE.pr.llr.1)],kernel=kernel)    
@

<<LLR.fuzzy.y,eval=TRUE,echo=FALSE,results='hide',cache=TRUE>>=
MSE.llr.0 <- sapply(MSE.grid,MSE.llr,y=y,D=Z,x=yB,kernel=kernel,d=0)
MSE.llr.1 <- sapply(MSE.grid,MSE.llr,y=y,D=Z,x=yB,kernel=kernel,d=1)

y.Z0.llr <- llr(y[Z==0],yB[Z==0],yB[Z==0],bw=MSE.grid[MSE.llr.0==min(MSE.llr.0)],kernel=kernel)    
y.Z1.llr <- llr(y[Z==1],yB[Z==1],yB[Z==1],bw=MSE.grid[MSE.llr.1==min(MSE.llr.1)],kernel=kernel)    
@
...
<<rdd.fuzzy.local.iv,eval=TRUE,echo=FALSE,results='hide'>>=
bw <- min(c(MSE.grid[MSE.pr.llr.0==min(MSE.pr.llr.0)],MSE.grid[MSE.pr.llr.1==min(MSE.pr.llr.1)],MSE.grid[MSE.llr.0==min(MSE.llr.0)],MSE.grid[MSE.llr.1==min(MSE.llr.1)]))
...
@
The optimal bandwidth is \bm{\Sexpr{bw}}.
...
<<plot.fuzzy.rdd.3,dependson='LLR.fuzzy.pscore',eval=TRUE,echo=FALSE,results='hide',fig.cap='Fuzzy RDD',fig.align='center',out.width='.65\\textwidth',fig.pos='htbp'>>=
plot(yB[Ds==0],y0[Ds==0],pch=1,xlim=c(4,10.5),ylim=c(4,10.5),xlab="yB",ylab="Outcomes")
points(yB[Ds==1],y[Ds==1],pch=3,col=col.obs)
points(yB[Z==0],y.Z0.llr,col='blue')
points(yB[Z==1],y.Z1.llr,col='blue',pch=3)
abline(v=log(param["barY"]),col=col.unobs)
legend(4,10,c('y0|D=0','y1|D=1','D'),pch=c(1,3,2),col=c(col.obs,'blue',col.unobs),ncol=1)
par(new=TRUE)
plot(yB,Ds,pch=2,col=col.unobs,xlim=c(5,11),xaxt="n",yaxt="n",xlab="",ylab="")
points(yB[Z==0],Pr.D0.llr,col='red')
points(yB[Z==1],Pr.D1.llr,col='red',pch=3)
axis(4)
mtext("D",side=4,line=3)
@
...
% not sure...
The estimated value of LATE(z) by simplified LLR IV is \Sexpr{delta.rdd.fuzzy.il}. The estimated 99\% sampling noise is: \Sexpr{2*qnorm((.99+1)/2)*sqrt(vcov(reg.fuzzy.rdd.local.iv)[2,2])}.
