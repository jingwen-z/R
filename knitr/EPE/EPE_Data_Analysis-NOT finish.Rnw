...
\small BY\\
\Large \textbf{Jingwen ZHENG}\\[0.5em]
...
\part{Introduction}
  ...
  As a student in M2 ERNA of Toulouse School of Economics, during the last half year of study, I'm really honoured to participate the course 'Econometrics of Program Evaluation'. In this course, I learnt some methods to estimate the effect of an interention on an outcome as the following outline.
...
\vspace{.5em}  
  \underline{The Two Fundamental Problems of Inference} indicate the Fundamental Problem of Causal Inference(FPCI) and the Fundamental Problem of Statistical Inference(FPSI). In the lecture \underline{Observational Methods of Causal Inference}, we learnt how to estimate the effect of an intervention on an outcome using observational methods, this method tries to correct for selection bias by accounting for as many observed confounders as possible. There are several ways to leverage on Selection on Observables to build estimators for TT; 'Parametric Observational Method: OLS', 'Nonparametric Observational Method: Matching' and 'Semiparametric Observational Method: Synthetic Control (SCM)'.

\vspace{.5em}  
  A \underline{natural experiment} is an empirical study in which individuals (or clusters of individuals) exposed to the experimental and control conditions are determined by nature or by other factors outside the control of the investigators, yet the process governing the exposures arguably resembles random assignment. Thus, natural experiments are observational studies and are not controlled in the traditional sense of a randomized experiment. In the lecture 'Natural Experiments', we learnt how to estimate the effect of an intervention on an outcome using natural experiments, this methods tries to correct for selection bias by finding situations analog to randomized experiments but due to the natural course of events. It covers three ways: Regression Discontinuity Designs (RDD), Difference in Differences (DID) and Instrumental Variables (IV). Natural experiments are most useful when there has been a clearly defined exposure involving a well defined subpopulation (and the absence of exposure in a similar subpopulation) such that changes in outcomes may be plausibly attributed to the exposure.

\vspace{.5em}  
  Besides, a \underline{Randomized Controlled Trial} is a type of scientific (often medical) experiment, where the people being studied are randomly allocated one or other of the different treatments under study. In the lecture 'Randomized Controlled Trials', we learnt how to estimate the effect of an intervention on an outcome using randomized experiments. We especially learnt the various types of designs and what can be recovered from them using which technique. RCTs are often used to test the efficacy or effectiveness of various types of medical intervention and may provide information about adverse effects, such as drug reactions. 

\vspace{.5em}  
  Moreover, we learnt how to choose the size of our sample (mainly in a RCT) and how to gauge the size of sampling noise before conducting a study in the lecture \underline{Power Analysis: Choosing Sample Size and Gauging Sampling Noise Ex Ante}. There are two ways; one is using test statistics, the other is gauging sampling noise or choosing sample size to reach a given sampling noise. 

\vspace{.5em}  
  By \underline{Placebo Tests and Power Analysis for Natural Experiments and Observational Studies}, we know how to test for the validity and estimate the power of observational studies and natural experiments. 

\vspace{.5em}  
  Finally, we get how to estimate the precision of an estimator by studying lecture \underline{Estimating Precision In Group Designs With Autocorrelated Errors}, when the treatment has been allocated to groups of observations whose outcomes are correlated.\\

\noindent{\textit{\textbf{Software}}}

\vspace{.5em}
  When writing a paper, slides or a report, we generally have a separate file for our statistical code and for our text (paper or slides). In general, the statistical files are in .R and the text in .tex. It's not convenient to use and is a waste of time.

\vspace{.5em}
  Thanks to this course, I master how to use \textbf{Knitr} package in RStudio in order to completely sidestep this problem by intertwining the .R code with .tex code within an .Rnw file.\\

\noindent{\textit{\textbf{For this project}}}

\vspace{.5em}
  Since I didn't choose Paul and Cesar's class, I didn't do the report for that course. In this report, I generate data as Mr. Chabé-Ferret did. Then I followed the questions that asked by prof on the moodle and did analysis with 'The Two Fundamental Problems of Inference', the method 'Observational Methods of Causal Inference' and the method 'Regression Discontinuity Designs(RDD)' of 'Natural Experiments'.

\vspace{.5em}  
  In this project, I firstly generated data, then I used different methods to analyse and estimate the effect of an interention on an outcome. I will explaine the result with graphs and text in the following part.
...
<<Pot.obs.out.FPCI,eval=TRUE,fig.cap='Potential outcomes and observed outcomes',fig.subcap=c('Potential outcomes','Observed outcomes'),fig.align='center',out.width='.45\\linewidth',echo=FALSE,results='hide',fig.pos='htbp'>>=
col.obs <- 'black'
col.unobs <- 'red'
...
Because of the large amount of data, here I just show the first 20 and last 20 individual level treatment effects as following, the completed individual level treatment effects are in 'Annex'. \\ 

\noindent{\Sexpr{it[1:20]} \\ ... \\ \Sexpr{it[1980:NC]}}
...
\centerline{$\Delta_{TT}^y =\bar{\alpha}+\theta\bar{\mu} -\theta\displaystyle\frac{\sigma^2_{\mu}}{\sqrt{\sigma^2_{\mu}+\sigma^2_{U}}}\frac{\displaystyle\phi\left(\frac{\bar{y}-\bar{\mu}}{\sqrt{\sigma^2_{\mu}+\sigma^2_{U}}}\right)}{\displaystyle\Phi\left(\frac{\bar{y}-\bar{\mu}}{\sqrt{\sigma^2_{\mu}+\sigma^2_{U}}}\right)}$}\\

<<theo.avg.tt,eval=TRUE,echo=FALSE>>=
theo.avg.tt<-delta.y.tt(param)
@

Thus, I get the theoretical average treatment effect is \textbf{\Sexpr{theo.avg.tt}}, which is smaller than the value of last question.
...
Here, I get the WW estimator in the sample is $\bm{\Sexpr{WW.esti}}$.
...
With the $\Delta^Y_{TT}$ and $\Delta^Y_{WW}$ that I got before, I can get the Selecton Bias is $\bm{\Sexpr{SB}}$.
...
Thus, I get the theoretical average treatment effect is $\bm{\Sexpr{theo.SB}}$, which is smaller than the value of last question.
...
Here, I get the BA estimator in the sample is $\bm{\Sexpr{BA.esti}}$.
...
With the $\Delta^Y_{TT}$ and $\Delta^Y_{BA}$ that I got before, I can get the Time Trend Bias is $\bm{\Sexpr{TB}}$.
...
\clearpage
      ...
      \subsection{Plot potential outcomes and observed outcomes}
<<Pot.obs.out.FPSI,eval=TRUE,fig.cap='Potential outcomes and observed outcomes',fig.subcap=c('Potential outcomes','Observed outcomes'),fig.align='center',out.width='.45\\textwidth',echo=FALSE,results='hide',fig.pos='htbp'>>=
col.obs <- 'black'
col.unobs <- 'red'
...
\noindent{\Sexpr{its[1:20]} \\ ... \\ \Sexpr{its[1480:NS]}}
...
Thus, I get the theoretical average treatment effect is \textbf{\Sexpr{theo.avg.tts}}, which is bigger than the value of last question.
...
Here, I get the WW estimator in the sample is $\bm{\Sexpr{WW.estiS}}$.

      \subsection{Compute the OLS of beta in $\bm{y_i = \alpha + \beta D_i + U_i}$}
According to the course, we learn the formula as following:\\ \\
\centerline{$\hat{\beta}_{OLS} = \displaystyle\frac{\frac{1}{N}\sum_{i=1}^N\left(Y_i-\frac{1}{N}\sum_{i=1}^NY_i\right)\left(D_i-\frac{1}{N}\sum_{i=1}^ND_i\right)}{\frac{1}{N}\sum_{i=1}^N\left(D_i-\frac{1}{N}\sum_{i=1}^ND_i\right)^2}$} \\
...
Thus, I get the value of $\hat{\beta}_{OLS}$ is $\bm{\Sexpr{betaOLS}}$, which is the same as WW.}
...
In order to compute the value of $\tilde{\epsilon}$, I firstly compute $\var{Y_i^1|D_i=1}$ and $\var{Y_i^0|D_i=0}$.}
...
According to R, I get $\bm{\var{Y_i^1|D_i=1}=\Sexpr{var1}}$ and $\bm{\var{Y_i^0|D_i=0}=\Sexpr{var0}}$.

<<probas,eval=TRUE,echo=FALSE>>=
proba1<-length(Dss[Dss==1])/NS
@

Moreover, I get $\bm{\Pr(D_i=1)=\Sexpr{proba1}}$.

<<tilde.epsilon.WW,eval=TRUE,echo=FALSE>>=
tilde.epsilon.WW<-qnorm((0.99+1)/2)*(1/NS)*sqrt((var1/proba1)+(var0/(1-proba1)))
@
Finally, I estimate the effect of sampling noise on WW with 99\% confidence is $\bm{\Sexpr{tilde.epsilon.WW}}$.
...
According to this graph, we observe that the result is similar to the one of prof.\\
It indicates that the sampling noise of the WW estimator decreases with increasing of sample size.
...
@
With the formula that I showed in the last question, I estimate the effect of sampling noise on WW with 95\% confidence is $\bm{\Sexpr{tilde.epsilon.WW.95}}$, which means the sampling noise has decreased.
...
In \textbf{1.2.8}, I have already gotten $\Pr(D_i=1)=\Sexpr{proba1}$ and $\tilde{\epsilon}=\Sexpr{tilde.epsilon.WW}$.
...
According to the calculating in R, I get $\bm{\var{yB|D_i=1}=\Sexpr{varyB1}}$ and $\bm{\var{yB|D_i=0}=\Sexpr{varyB0}}$.

<<tilde.N,eval=TRUE,echo=FALSE>>=
tilde.N<-(1/(tilde.epsilon.WW^2))*(qnorm((0.99+1)/2))^2*((varyB1/proba1)+(varyB0/(1-proba1)))
@

According to the formula above, I get $\bm{\tilde{N}=\Sexpr{tilde.N}}$, so the sample size should be $\bm{\Sexpr{tilde.N}}$.

Moreover, I would like to do the following graph like what prof did in the course.
...
According to this graph, we can get that the result is similar to the one of prof.\\
It indicates that with Central Limit Theorem (CLT), the precision increases with sample size, which is near to the precision of truth.
\clearpage
...
If I use the same variance $\var{y^B}$, then the sample size is $\bm{\Sexpr{N.bis}}$, which means the sample size increases.
...
The new sample size is $\bm{\Sexpr{N.new}}$, which is bigger than the sample size in 1.2.10, so the sample size increases.
...
\vspace{.5em}
Using R, I estimate the sample size with the sampling noise in question 10 is $\bm{\Sexpr{N.10}}$, the one with the sampling noise in question 12 is $\bm{\Sexpr{N.12}}$.

\vspace{.5em}
Now, I will compare the sample size in different situation.

\vspace{.5em}
With 99\% confidence, when the sampling noise is $\Sexpr{tilde.epsilon.WW}$, the sample size is $\Sexpr{tilde.N}$; when the sampling noise is 0.002087, the sample size is $\Sexpr{N.new}$.

\vspace{.5em}
With 95\% confidence, when the sampling noise is $\Sexpr{tilde.epsilon.WW}$, the sample size is $\Sexpr{N.10}$; when the sampling noise is 0.002087, the sample size is $\Sexpr{N.12}$.

\vspace{.5em}
Comparing the results above, we can easily observe that for the same confidence level, sample size decreases with sampling noise; for the same sampling noise, sample size increases  with confidence level.
...
<<plot.ols.estim.step.0-1,eval=TRUE,fig.cap='compute the OLS estimator step by step',fig.subcap=c('Step 0','Step 1'),fig.align='center',out.width='.45\\textwidth',echo=FALSE,results='hide',fig.pos='htbp'>>=
# Step 0
col.obs <- 'black'
col.unobs <- 'red'
lty.obs <- 1
...
<<plot.ols.estim.step.2-3,eval=TRUE,fig.cap='compute the OLS estimator step by step',fig.subcap=c('Step 2','Step 3'),fig.align='center',out.width='.45\\textwidth',echo=FALSE,results='hide',fig.pos='htbp'>>=
# Step 2
...
<<plot.ols.estim.step.4,eval=TRUE,echo=FALSE,results='hide',fig.cap='Step 4',fig.align='center',out.width='.49\\textwidth',out.height='.49\\textwidth',fig.width=5.5,fig.height=5.5,fig.pos='htbp'>>=
...
Then, I'll compute the OLS estimator directly.According to the course, I get the following assumptions:\\
...
tt<-mean(y1o-y0o)
@
According to R, I get $\bm{\Delta^Y_{TT}=\Sexpr{tt}}$, which means $\bm{\Delta^Y_{WWOLS(X)}=\Sexpr{tt}}$.

Moreover, thanks to another assumption \textbf{Parametric functional form for the Treated} and theorem \textbf{WWOLS is OLS}, I get\\
\centerline{$\hat{\delta}_{OLS} = \hat{\Delta^Y_{WWOLS(X)}}$}

So I get the OLS estimator $\bm{\hat{\delta}_{OLS} = \hat{\Delta^Y_{WWOLS(X)}}=\Sexpr{tt}$.

Furthermore, I get the following graph:
...
<<plot.ols.estim.direct,eval=TRUE,echo=FALSE,results='hide',fig.cap='Direct OLS Estimation',fig.align='center',out.width='.65\\textwidth',fig.pos='htbp'>>=
plot(1, type="n", xlab="", ylab="", xlim=c(0.3,0.9), ylim=c(0, 10))
abline(v=delta.y.tt(param),col=col.unobs,lty=lty.unobs)
...
Now I compare their values with the theoretical TT. According to the course, we learnt the theoretical TT with the following formula:\\
\centerline{$\Delta_{TT}^y =\bar{\alpha}+\theta\bar{\mu} -\theta\displaystyle\frac{\sigma^2_{\mu}}{\sqrt{\sigma^2_{\mu}+\sigma^2_{U}}}\frac{\displaystyle\phi\left(\frac{\bar{y}-\bar{\mu}}{\sqrt{\sigma^2_{\mu}+\sigma^2_{U}}}\right)}{\displaystyle\Phi\left(\frac{\bar{y}-\bar{\mu}}{\sqrt{\sigma^2_{\mu}+\sigma^2_{U}}}\right)}$}
...
Thus, I get the theoretical TT is \textbf{\Sexpr{theo.tt}}, which is smaller than the value that I calculated just now.
...
The robust standard error using HC1 is thus $\bm{\Sexpr{sqrt(ols.direct.HC1[3,3])}}$. Then, I'll compute the sampling noise and show it in the following graph.
...
It indicates that the precision of the OLS estimator with 99\% confidence increases with sample size.
...
@
According to R, I get the propensity score is $\bm{\Sexpr{propen.score}}$.
...
According to this graph, we can observe that some treated and untreated individuals are overlapped, which indicates common support holds.\\
...
According to this graph, we can observe that there exists lots of common support between the density of treated and untreated individuals.
...
<<plot.LLR.trimming,dependson='LLR.trim',eval=TRUE,echo=FALSE,results='hide',fig.cap='LLR Matching with Trimming',fig.align='center',out.width='.5\\textwidth',out.height='.5\\textwidth',fig.width=7,fig.height=7,fig.pos='htbp'>>=
plot(1, type="n", xlab="", ylab="", xlim=c(0.3,0.9), ylim=c(0, 10))
abline(v=delta.y.tt.com.supp(param),col=col.unobs,lty=lty.unobs)
...
Since it will take lots of time to do the bootstrap for larger sample, so here I just take a sample of 100 individuals as an example. Obviously, because of the sample size is not big enough, there exists a large sampling noise.
...
\vspace{.5em}     
RDD uses situations where there is a discontinuity in the probability of receiving the treatment. If there is also a discontinuity in outcomes, it is interpreted as the effect of the treatment.
...
<<identification.sharp.ols.1,eval=FALSE,echo=FALSE,results='hide',fig.cap='the discontinuity in the linear sample using OLS',fig.align='center',out.width='.65\\textwidth',fig.pos='htbp'>>=
...
According to R, I get $\esph{y^1_i|y^B_i=\bar{y}}=$\Sexpr{y1.bary.pred} and $\esph{y_i^0|y_i^B=\bar{y}}=$\Sexpr{y0.bary.pred}, so the estimated TT is $\hat{\Delta^{y}_{RDDOLS}}=$\Sexpr{delta.y.rddols}.

Then I calculate $\bm{TT(z)}$ with the following formula:\\

\centerline{$\Delta^y_{TT}(\bar{z}) = \bar{\alpha} + \theta\bar{\mu}+\theta\displaystyle\frac{\sigma^2_{\mu}}{\sigma^2_{\mu}+\sigma^2_{U}}(\bar{y}-\bar{\mu})$}
...
\begin{itemize}
  \item Compute the optimal bandwidth using CV
\end{itemize}
...
<<plot.rdd.llr.cv,eval=TRUE,echo=FALSE,results='hide',fig.cap='Cross Validation Results',fig.align='center',out.width='.5\\textwidth',out.height='.5\\textwidth',fig.width=7,fig.height=7,fig.pos='htbp'>>=
...
<<plot.rdd.llr.nonlin.1,eval=TRUE,echo=FALSE,results='hide',fig.cap='RDD LLR',fig.align='center',out.width='.5\\textwidth',out.height='.5\\textwidth',fig.width=7,fig.height=7,fig.pos='htbp'>>=
...
By doing this project, I have a better understanding in the theoreme and estimating the effect of an interention on an outcome in different methods, which enrich my knowledge on econometrics.
...
\vspace{.5em}
In Chapter 2, I analysed in Parametric and Nonparametric Observational Methods, which are OLS and Matching, respectively. For Parametric Observational Methods, I computed the OLS estimators, the robust standard errors and sampling noise. Then I observed that with the increasing of sample size, the sampling noise becomes much smaller (the precision of the OLS estimator with increases with sample size). For Nonparametric Observational Methods, I computed the LLR estimator on the sample with common support and understood how to choose the bandwidth. Moreover, I computed the standard errors and sampling noise using the
bootstrap.
...
\vspace{.5em}  
Moreover, I should also use other methods that been taught in the class to do the analysis afterwords, and try to use the real data as well. So that I can understand the application of the theritical knowledge even better.
