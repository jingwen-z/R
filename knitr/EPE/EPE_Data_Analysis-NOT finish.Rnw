...
\vspace{.5em}
  Since I didn't choose Paul and Cesar's class, I didn't do the report for that course. In this report, I generate data as Mr. Chabé-Ferret did. I did analysis with "The Two Fundamental Problems of Inference", the method "Observational Methods of Causal Inference" and the method "Regression Discontinuity Designs(RDD)" of "Natural Experiments".
...
According to this graph, we can observe that there is an overlapped part, which indicates common support holds.\\
...
<<identification.sharp.yB.3,eval=TRUE,echo=FALSE,results='hide',fig.align='center',out.width='.65\\textwidth',fig.pos='htbp'>>=
plot(yB[Ds==0],y[Ds==0],pch=1,xlim=c(4,10.5),ylim=c(4,10.5),xlab="yB",ylab="Outcomes")
points(yB[Ds==1],y[Ds==1],pch=3)
points(yB[Ds==0],reg.yB.00$fitted.values,col='blue',pch=1)
...
<<identification.sharp.ols.3,eval=TRUE,echo=FALSE,results='hide',fig.cap='the discontinuity in the linear sample using OLS',fig.align='center',out.width='.65\\textwidth',fig.width=8,fig.height=8,fig.pos='htbp'>>=
plot(yBo[Dso==0],yo[Dso==0],pch=1,xlim=c(4,10.5),ylim=c(4,10.5),xlab="yB",ylab="Outcomes")
points(yBo[Dso==1],yo[Dso==1],pch=3)
points(yBo[Dso==0],reg.ols.00$fitted.values,col='blue',pch=1)
...
\noindent{According to R, I get $\esph{y^1_i|y^B_i=\bar{y}}=$\Sexpr{y1.bary.pred} and $\esph{y_i^0|y_i^B=\bar{y}}=$\Sexpr{y0.bary.pred}, so the estimated TT is $\hat{\Delta^{y}_{RDDOLS}}=$\Sexpr{delta.y.rddols}.\\
...
\noindent{So $\bm{TT(z)=\Sexpr{delta.y.tt.z(param)}}$.\\
Moreover, I show the graph of the results of RDD using OLS as following:}

<<plot.RDD.sharp.ols,eval=FALSE,echo=FALSE,results='hide',fig.cap='Sharp RDD Using OLS',fig.align='center',out.width='.65\\textwidth',fig.pos='htbp'>>=
...
<<plot.rdd.llr.cv,eval=TRUE,echo=FALSE,results='hide',fig.cap='Cross Validation Results',fig.align='center',out.width='.5\\textwidth',out.height='.5\\textwidth',fig.width=8,fig.height=8,fig.pos='htbp'>>=
...
<<plot.rdd.llr.nonlin.1,eval=TRUE,echo=FALSE,results='hide',fig.cap='RDD LLR',fig.align='center',out.width='.5\\textwidth',out.height='.5\\textwidth',fig.width=8,fig.height=8,fig.pos='htbp'>>=
...
@
The optimal bandwidth is \bm{0.3}.
...
% not sure...
% we have gotten delta.rdd.fuzzy.il<-0.005781747 and 2*qnorm((.99+1)/2)*sqrt(vcov(reg.fuzzy.rdd.local.iv)[2,2])<-3.360884
% The estimated value of LATE(z) by simplified LLR IV is $\bm{0.005781747}$. The estimated 99\% sampling noise is $\bm{3.360884}$.
