...
lat_list <- lapply(excel_sheets("latitude.xlsx"),
                   read_excel,
                   path = "latitude.xlsx")
str(lat_list)
...
latitude_sel <- read_excel("latitude.xlsx", sheet = 2, col_names = F, skip = 21)
...
path <- "urbanpop.xls"
urban_sheet1 <- read.xls(path, sheet = 1, stringsAsFactors = F)
urban_sheet2 <- read.xls(path, sheet = 2, stringsAsFactors = F)
