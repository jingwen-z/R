...
#         1 
#  703.1869 
# With a nose width of 250mm, your prediction tells us Skippy has a nose length of 
# around 700mm.
...
# Find the ground truth labels and then compare them with predictions 
# that made with lm_wb_log.
...
# x_pred: predictor values of the new observations (the cgdp column of world_bank_test)
...
## I'll calculate the RMSE of the test set for the simple linear, log-linear and k-NN regression. 
## Have a look at the results, which regression approach performs the best?

# Calculate RMSE on the test set for simple linear model
sqrt(mean( (test_output_lm - world_bank_test$urb_pop) ^ 2))  # 17.41897

# Calculate RMSE on the test set for log-linear model
sqrt(mean( (test_output_lm_log - world_bank_test$urb_pop) ^ 2))  # 15.01911

# Calculate RMSE on the test set for k-NN technique
sqrt(mean( (test_output_knn - world_bank_test$urb_pop) ^ 2))  # 16.10026

# The log-linear model seems to give the best RMSE on the test set!
# That was all for regression. 
